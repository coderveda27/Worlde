...
def get_character_status(answer: str, guess: str, i: int) -> str:
    ...
    True
    """
    if is_wrong_position_char(answer, guess, i):
        return WRONG_POSITION
    elif is_incorrect_char(answer, guess, i):
        return INCORRECT
    else:
        return CORRECT
...
def get_guess_status(answer: str, guess: str) -> list[str]:
    ...
    True
    """
    return [get_character_status(answer, guess, i) for i in range(0, len(guess))]
...
def get_guesses_statuses(answer: str, guesses: list[str]) -> list[list[str]]:
    ...
    True
    """
    return [get_guess_status(answer, i) for i in guesses]
...
def part3a_example(answer: str, guesses: list[str]) -> None:
    ...
    """
    # get_guesses_statuses(answer, guesses)
...
def is_correct_single(word: str, guess: str, status: list[str]) -> bool:
    ...
    False
    """
    return status == get_guess_status(word, guess)
...
def is_correct_multiple(word: str, guesses: list[str], statuses: list[list[str]]) -> bool:
    ...
    True
    """
    return all([is_correct_single(word, guesses[i], statuses[i]) for i in range(0, len(guesses))])
...
def find_correct_answers(word_set: set[str],
    ...
    ['later', 'liter']
    """
    return sorted([words for words in word_set if is_correct_multiple(words, guesses, statuses)])
...
def part3b_example(word_set_file: str, guesses: list[str], statuses: list[list[str]]) -> None:
    ...
    correct_answers = find_correct_answers(word_set, guesses, statuses)
    a2_visualizer.draw_wordle_answers(correct_answers, guesses, statuses)
...
def find_correct_guesses_single(word_set: set[str], answer: str, status: list[str]) -> list[str]:
    ...
    ['tiger', 'tower']
    """
    return sorted([word for word in word_set if is_correct_single(answer, word, status)])
...
def find_correct_guesses_multiple(word_set: set[str],
    ...
    """
    return cartesian_product_general([find_correct_guesses_single(word_set, answer, status) for status in statuses])
...
def part3c_example(word_set_file: str, answer: str, statuses: list[list[str]]) -> None:
    ...
    new_word_set_guesses = find_correct_guesses_multiple(word_set, answer, statuses)
    a2_visualizer.draw_wordle_guesses(answer, new_word_set_guesses, statuses)
...
def information_score(answer: str, guess: str) -> float:
    ...
    - answer != ''
    """
    current_status = get_guess_status(answer, guess)
    return current_status.count(CORRECT) + 0.5 * current_status.count(WRONG_POSITION)
...
def find_correct_answers_and_scores(word_set: set[str],
    ...
    """
    correct_answer_words = find_correct_answers(word_set, guesses, statuses)
    info_scores = [information_score(correct_answer_words[i], guesses[i]) for i in range(0, len(guesses))]
    average_info_scores = sum(info_scores) / len(info_scores)

    dict_scores = {correct_answer_words: i for i in average_info_scores}
    return dict_scores
